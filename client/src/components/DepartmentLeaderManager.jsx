...
import { Crown, Edit3, Plus, Power, RefreshCw, Search, Trash2, X } from "lucide-react";
import { api } from "../api";
import { useLanguage } from "../i18n";

export default function DepartmentLeaderManager() {
  const { t } = useLanguage();
  ...
  const [form, setForm] = useState({ person_id: "", person_name: "", person_union_id: "", leader_departments: [] });
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get("/admin/department-leaders", { params });
  ...
  }, [page, pageSize, keyword, filterEnabled, t]);
  ...
  useEffect(() => {
    ...
    if (form.leader_departments.length === 0) {
      setSearchResults([]);
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await api.get("/admin/department-leaders/search", {
          params: {
            keyword: searchKeyword,
            department_names: form.leader_departments.join(","),
            pageSize: 20
          }
        });
  ...
  }, [showModal, editId, searchKeyword, form.leader_departments]);

  function openCreate() {
    setEditId(null);
    setForm({ person_id: "", person_name: "", person_union_id: "", leader_departments: [] });
    setSearchKeyword("");
    setSearchResults([]);
    setShowModal(true);
  }
  ...
  function openEdit(item) {
    ...
    setForm({
      ...
      leader_departments: item.leader_departments || []
    });
    setSearchKeyword(item.person_name || "");
    setSearchResults([]);
    setShowModal(true);
  }
  ...
  function toggleDept(dept) {
    setForm((current) => {
      const next = current.leader_departments.includes(dept)
        ? current.leader_departments.filter((item) => item !== dept)
        : [...current.leader_departments, dept];
      return {
        ...current,
        leader_departments: next,
        person_id: editId ? current.person_id : "",
        person_name: editId ? current.person_name : "",
        person_union_id: editId ? current.person_union_id : ""
      };
    });
    if (!editId) {
      setSearchResults([]);
  ...
  async function handleSave() {
    ...
    if (form.leader_departments.length === 0) { setError(t("admin.selectLeaderDepartment")); return; }
    setSaving(true);
    setError("");
    try {
      const body = { department_names: form.leader_departments };
      if (editId) {
        await api.put(`/admin/department-leaders/${editId}`, body);
      } else {
        body.person_id = form.person_id;
        await api.post("/admin/department-leaders", body);
      }
      closeModal();
      loadList();
    } catch (err) {
  ...
  async function handleToggle(item) {
    try {
      await api.patch(`/admin/department-leaders/${item.id}/toggle`, { is_enabled: !item.is_enabled });
      loadList();
    } catch (err) {
      setError(err.response?.data?.message || t("admin.operationFailed"));
    }
  ...
  async function handleDelete(item) {
    if (!confirm(t("admin.confirmDeleteLeader", { name: item.person_name || item.person_id }))) return;
    try {
      await api.delete(`/admin/department-leaders/${item.id}`);
      loadList();
    } catch (err) {
    ...
    <div className="space-y-4">
        ...
        <div className="relative z-10 flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="ai-chip mb-4">
              ...
              {t("admin.leaderConfig")}
            </div>
            <h1 className="text-2xl font-semibold tracking-tight text-ai-title">{t("admin.leaderConfigTitle")}</h1>
            <p className="mt-3 max-w-2xl text-sm leading-7 text-ai-body">{t("admin.leaderConfigDesc")}</p>
          </div>
          <div className="flex gap-2">
            <button type="button" onClick={openCreate} className="primary-button">
              <Plus size={16} />
              {t("admin.createLeader")}
            </button>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex h-10 items-center gap-2 rounded-xl border border-ai-border bg-white px-3">
          <Search size={16} className="shrink-0 text-ai-muted" />
          <input
            value={keyword}
        ...
        <select value={filterEnabled} onChange={(event) => { setFilterEnabled(event.target.value); setPage(1); }} className="soft-input h-10 text-sm">
          ...
          <table className="soft-table w-full min-w-[700px]">
            <thead>
              <tr>
                ...
                <th>{t("admin.leaderDepartments")}</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr><td colSpan="5" className="px-6 py-12 text-center text-ai-body">{t("common.loading")}</td></tr>
              ) : items.length === 0 ? (
                <tr><td colSpan="5" className="px-6 py-12 text-center text-ai-body">{t("admin.noLeaderRecords")}</td></tr>
              ) : (
                items.map((item) => (
                  <tr key={item.id}>
                    <td>
                    ...
                    <td>
                      <div className="flex flex-wrap gap-1">
                        {(item.leader_departments || []).map((dept) => (
                          <span key={dept} className="inline-block rounded-full bg-ai-primary/10 px-2 py-0.5 text-xs font-medium text-ai-primary">{dept}</span>
                        ))}
                      </div>
                    </td>
                    <td>
      ...
      {showModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={closeModal}>
          <div className="motion-popover max-h-[90vh] w-full max-w-[600px] overflow-y-auto rounded-[24px] border border-white/80 bg-white/95 p-6 shadow-[0_28px_80px_rgba(17,17,17,0.16)] backdrop-blur-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-5 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-ai-title">{editId ? t("admin.editLeader") : t("admin.createLeader")}</h2>
            ...
            <div className="space-y-5">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-ai-body">{t("admin.stepChooseLeaderDepartments")}</label>
                <div className="max-h-[200px] space-y-1 overflow-y-auto rounded-xl border border-ai-border bg-ai-bg p-2">
                  {departments.map((dept) => (
                    <label key={dept} className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-white">
                      <input
                        type="checkbox"
                        checked={form.leader_departments.includes(dept)}
                ...
                {form.leader_departments.length > 0 ? (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {form.leader_departments.map((dept) => (
                      <span key={dept} className="inline-flex items-center gap-1 rounded-full bg-ai-primary/10 px-2 py-0.5 text-xs font-medium text-ai-primary">
                        {dept}
                        <button type="button" onClick={() => toggleDept(dept)} className="hover:text-red-500"><X size={12} /></button>
                      </span>
                    ))}
                  </div>
                ) : null}
              ...
              {!editId ? (
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-ai-body">{t("admin.stepChooseLeader")}</label>
                  <div className="mb-3 flex h-10 items-center gap-2 rounded-xl border border-ai-border bg-white px-3">
                    <Search size={16} className="shrink-0 text-ai-muted" />
                    <input
                      value={searchKeyword}
                      onChange={(event) => { setSearchKeyword(event.target.value); setForm((current) => ({ ...current, person_id: "", person_name: "" })); }}
                      className="h-full w-full border-0 bg-transparent text-sm outline-none"
                      placeholder={form.leader_departments.length ? t("admin.searchNameOrId") : t("admin.selectDepartmentsFirst")}
                      disabled={form.leader_departments.length === 0}
                    />
                  </div>
                  {form.leader_departments.length === 0 ? (
                    <div className="rounded-xl bg-ai-bg px-3 py-4 text-center text-sm text-ai-body">{t("admin.selectLeaderDepartmentsFirstDesc")}</div>
                  ) : searchLoading ? (
                    ...
                    <div className="max-h-[200px] space-y-1 overflow-y-auto rounded-xl border border-ai-border bg-ai-bg p-1">
                      {searchResults.map((person) => (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            setForm((current) => ({ ...current, person_id: person.id, person_name: person.name, person_union_id: person.union_id || "" }));
                          ...
                          className={`flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-left text-sm transition duration-200 ${
                            form.person_id === person.id ? "bg-ai-primary/10 text-ai-primary" : "text-ai-body hover:bg-white"
                          }`}
                        >
                          <div>
                            <div className="font-semibold text-ai-title">{person.name}</div>
                            <div className="mt-0.5 text-xs text-ai-muted">{person.union_id || ""}</div>
                          </div>
                        </button>
                      ))}
                    </div>
              ...
              <button
                type="button"
                onClick={handleSave}
                disabled={saving || (!editId && !form.person_id) || form.leader_departments.length === 0}
